    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex-1 max-w-7xl mx-auto px-4 py-8 w-full">
        ...
        {!!sourceStats && (
          <section className="mb-6 rounded-xl border border-gray-200 bg-white p-4">
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div>
                <p className="text-xs uppercase tracking-wide text-rose-700 mb-2">Domestic</p>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(sourceStats.domesticByPlatform || {})
                    .sort((a, b) => b[1] - a[1])
                    .slice(0, 10)
                    .map(([platform, count]) => (
                      <span key={`dom-${platform}`} className="px-2 py-1 rounded bg-rose-50 text-rose-800 text-xs border border-rose-200">
                        {platform}: {count}
                      </span>
                    ))}
                </div>
              ...
              <div>
                <p className="text-xs uppercase tracking-wide text-rose-700 mb-2">International</p>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(sourceStats.internationalByPlatform || {})
                    .sort((a, b) => b[1] - a[1])
                    .slice(0, 10)
                    .map(([platform, count]) => (
                      <span key={`intl-${platform}`} className="px-2 py-1 rounded bg-rose-50 text-rose-800 text-xs border border-rose-200">
                        {platform}: {count}
                      </span>
                    ))}
                </div>
        ...
        <section className="mb-10 rounded-xl border border-rose-200 bg-rose-50/40 p-4 md:p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-xl font-semibold text-rose-800">Domestic Sellers</h2>
              <p className="text-xs text-rose-700">Bangladesh platforms and local marketplaces</p>
            </div>
        ...
        <section className="mb-10 rounded-xl border border-rose-200 bg-rose-50/40 p-4 md:p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-xl font-semibold text-rose-800">International Sellers</h2>
              <p className="text-xs text-rose-700">Cross-border global marketplaces</p>
            </div>
