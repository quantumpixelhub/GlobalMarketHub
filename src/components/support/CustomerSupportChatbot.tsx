...
export const CustomerSupportChatbot: React.FC<ChatbotProps> = ({
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className={`fixed bottom-6 right-6 p-4 bg-teal-500 text-white rounded-full shadow-lg hover:bg-teal-600 transition transform hover:scale-110 z-40 ${className}`}
  ...
  return (
    ...
    >
      ...
      <div
        className={`${
          isEscalated ? 'bg-rose-600' : 'bg-rose-600'
              ...
              {msg.isThinking ? (
                <div className="flex items-center gap-1">
                  ...
                  <span className="inline-flex gap-0.5">
                    ...
                    <span className="w-1.5 h-1.5 bg-rose-600 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }} />
                    <span className="w-1.5 h-1.5 bg-rose-600 rounded-full animate-bounce" style={{ animationDelay: '0.2s' }} />
                  </span>
                </div>
              ) : (
                <>
      ...
      <div className="border-t border-gray-200 p-3 bg-white rounded-b-lg">
        {isEscalated && (
          <div className="text-xs text-rose-600 bg-rose-50 p-2 rounded mb-2 border border-rose-200">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-600 text-sm disabled:bg-gray-100"
          />
          <button
            onClick={handleSendMessage}
            disabled={isLoading || !inputValue.trim() || isEscalated}
            className="bg-teal-500 text-white p-2 rounded-lg hover:bg-teal-600 disabled:bg-gray-400 transition"
